...
* Final figures, reports, and processed output
...
Use scratch areas (for example `/home/scratch3` or `/home/scratch12`) for temporary or high-volume data generated during active work.
...
Scratch storage is often suitable when you need more space or when workflows generate many files. However, **there is no backup**, so files stored there are at your own risk.

Local disk on compute nodes (`/tmp`)
====================================

Each compute node also provides local temporary disk space mounted at `/tmp`.
...
Files in `/tmp` are local to the compute node, are not shared across nodes, and should be considered temporary. Important data should be copied elsewhere before the job finishes. This option is thus only applicable for multinode jobs if the software is able to handle node-local scratch folders.
...
If losing the file would be a problem, store it in backed-up home or project storage.

If the file is temporary or can be recreated, scratch storage or `/tmp` is usually the better choice.
...
.. code-block:: bash
    ...
    SCRATCHDIR='/home/scratch12/stly'
    ...
    cp -v ${SLURM_SUBMIT_DIR}/{INCAR,POSCAR,POTCAR,KPOINTS} $CALCDIR/
    cd $CALCDIR
...
Same objective using ASE
------------------------

Using ASE you need to tell the calculator where to put temporary files
or save calculation results, the exact method may differ for other
calculators.
...
.. code-block:: python
   ...
   SCRATCH_FOLDER = Path('/home/scratchX/<username>/')
...
.. code-block:: bash
   ...
   cp -v ${SLURM_SUBMIT_DIR}/{INCAR,POSCAR,POTCAR,KPOINTS} $TMPDIR/
   cd $TMPDIR
